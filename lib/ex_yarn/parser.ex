...
defmodule ExYarn.Parser do
  ...
  defp do_parse(%__MODULE__{current_token: token}) do
    throw({:message, "Unknown token", :token, token})
  end
  ...
  defp parse_object(parser, keys, was_colon) do
    cond do
      ...
      true ->
        throw({:message, "Invalid value type", :token, parser.current_token})
    end
  end
  ...
  defp parse_keys(%__MODULE__{current_token: %Token{type: :comma}} = parser, keys) do
    ...
    case parser.current_token.type do
      ...
      _ ->
        throw({:message, "Expected string", :token, parser.current_token})
    end
  end
  ...
  defp on_comment(%__MODULE__{current_token: current_token}) do
    throw({:message, "Expected token value to be a string", :token, current_token})
  end
    ...
    case Regex.run(@version_regex, comment) do
      ...
      captures ->
        ...
        if String.to_integer(version) > @lockfile_version do
          throw({:message, "Lockfile version #{version} is not supported", :token, token})
        else
          :ok
        end
    end
